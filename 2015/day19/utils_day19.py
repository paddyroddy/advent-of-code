...
def _create_list_of_elements(input_string: str) -> list[str]:
    """
...
def _find_matching_molecules(
    input_string: str, df_rules: pd.DataFrame, position: int, old_molecule: str
) -> set[str]:
    """
    ...
    return {
        _swap_molecule_in_string(input_string, position, old_molecule, new_molecule)
        for new_molecule in replaced_elements
    }


def compute_distinct_molecules(rules: pd.DataFrame, input_string: str) -> set[str]:
    """
    ...
    elements_list = _create_list_of_elements(input_string)

    # loop through elements in list
    for element in elements_list:
        new_strings = _find_matching_molecules(input_string, rules, position, element)
